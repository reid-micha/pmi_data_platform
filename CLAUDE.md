...
> - **同日 worker/queue 層落地（CORR-4.6 + 8.1 + 6.5 + SHIP-3.4，全改 Postgres 版——reid 拍板不引 Redis / Temporal）**：`core_jobs` queue（SKIP LOCKED claim + LISTEN/NOTIFY + dedupe + retry/backoff + stale 回收；alembic 0009）＋ `pmi-workers worker` 長駐 loop（compose `pmi-worker`）＋ cron → enqueue（crontab 全改 `pmi-workers enqueue`）＋ §3.2 on-demand（`GET /score?max_age_s=&wait_s=` 202+job / 同步等、`POST /score/refresh`、`GET /jobs/{id}`；**cache = `ts_index_scores` 最新列，無 Redis**）＋ WS-trigger re-eval（新 `pmi-ingest-ws` service → `reeval-market` → per-index fan-out，三層 storm control）＋ durable workflow（`core_workflow_runs/steps` step-checkpoint，crash → queue retry → replay 吃 cache）＋ **backtest**（`POST /indexes/{id}/backtest` → `GET /workflows/{id}/csv`；CLI `pmi-workers backtest --wait` 吐 CSV）。驗證：workers 測試 10/10、api 46/46、NOTIFY 喚醒 13ms、500-market on-demand tick 9s、7-day backtest 7/7 steps、WS 真連線載入 811 component markets。
...
| §3.2 On-demand 計算 | Redis cache + Arq enqueue | ✅ **2026-06-11 落地（Postgres 版，無 Redis）**：cache = `ts_index_scores` 最新列；stale → `core_jobs` enqueue（dedupe）→ 202+job_id 或 `wait_s` 同步等；`POST /score/refresh`、`GET /jobs/{id}` | `pmi-api/pmi_api/routes/{indexes,jobs}.py`、`pmi-core/pmi_core/queue.py` |
...
| §7 Arq + Temporal | 雙 worker 混合 | ✅ **2026-06-11 落地（Postgres 版——reid 拍板不引 Redis / Temporal）**：Arq 角色 = `core_jobs` queue（SKIP LOCKED + LISTEN/NOTIFY + dedupe + retry + stale 回收）+ `pmi-workers worker` 長駐 loop（compose `pmi-worker`，可水平 scale）；cron 改 enqueue；WS-trigger `reeval-market` fan-out。Temporal 角色 = `core_workflow_runs/steps` step-checkpoint durable workflow（crash → queue retry → replay 吃 cache），旗艦 backtest（含 CSV）。真 Temporal 等 signals / timers / multi-worker fan-out 需求出現再回頭（§13） | `pmi-core/pmi_core/{queue,workflow}.py`、`pmi-workers/pmi_workers/worker.py`、`pmi-core/pmi_core/engine/backtest.py`、`pmi-workers/cron/crontab` |
